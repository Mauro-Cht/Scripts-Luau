-- Matsune Fluent — Rewritten UI version
-- This file executes the original Matsune logic but suppresses its original ScreenGui
-- and exposes controls via Fluent UI.

local ok, Fluent = pcall(function()
    return loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))()
end)
if not ok or not Fluent then warn("Failed to load Fluent"); return end
local SaveManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/SaveManager.lua"))()
local InterfaceManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/InterfaceManager.lua"))()

local Window = Fluent:CreateWindow({
    Title = "Matsune (Fluent UI)",
    SubTitle = "Rewritten — ChatGPT",
    TabWidth = 160,
...
-- helper
local function safeSetG(k,v) pcall(function() _G[k]=v end) end

-- Embedded original script payload (will be executed with GUI suppression)
local original_payload = [[==MATSUNE_PAYLOAD==
local MatsuneA1 = {};
...
-- Execute payload while preventing creation of new ScreenGui that would duplicate UI.
local function run_payload_safely()
    if run_payload_safely._ran then return end
    run_payload_safely._ran = true
    local Players = game:GetService("Players")
    local LocalPlayer = Players.LocalPlayer
    -- Backup
    local success, origInstanceNew = pcall(function() return Instance.new end)
    local origNew = origInstanceNew
    -- Override Instance.new to redirect ScreenGui creations into a Folder under PlayerGui (so original GUI elements won't show)
    if origNew then
        Instance.new = function(className, parent)
            if className == "ScreenGui" then
                local folder = origNew("Folder")
                folder.Name = "_SuppressedGUIs_"
                if LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui") then
                    folder.Parent = LocalPlayer.PlayerGui
                else
                    folder.Parent = game:GetService("CoreGui") or workspace
                end
                return folder
            end
            return origNew(className, parent)
        end
    end

    local fn, err = loadstring(original_payload)
    if not fn then
        Fluent:Notify({Title="Error", Content="Failed to compile payload: "..tostring(err), Duration=8})
        -- restore
        if origNew then Instance.new = origNew end
        return
    end

    local ok, res = pcall(fn)
    if not ok then
        Fluent:Notify({Title="Error", Content="Error running payload: "..tostring(res), Duration=8})
    else
        Fluent:Notify({Title="Matsune", Content="Payload executed (original GUI suppressed).", Duration=6})
    end
...
-- Run payload automatically so its functions/variables are available for Fluent controls
run_payload_safely()
-- Auto-generated controls for detected _G keys (and some priority ones)
local FastAttackToggle = Tabs.Auto:AddToggle("FastAttack", {Title = "FastAttack", Default = false})
FastAttackToggle:OnChanged(function()
    safeSetG("FastAttack", FastAttackToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["FastAttack"]) == "function" and FastAttackToggle.Value then _G["FastAttack"]() end
    end)
end)

local AutoClickToggle = Tabs.Auto:AddToggle("AutoClick", {Title = "AutoClick", Default = false})
AutoClickToggle:OnChanged(function()
    safeSetG("AutoClick", AutoClickToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["AutoClick"]) == "function" and AutoClickToggle.Value then _G["AutoClick"]() end
    end)
end)

local ClickDelaySlider = Tabs.Auto:AddSlider("ClickDelay", {Title = "ClickDelay", Min = 0, Max = 1, Default = 0, Rounding = 2})
ClickDelaySlider:OnChanged(function(v) safeSetG("ClickDelay", v) end)

local AutoFarmToggle = Tabs.Auto:AddToggle("AutoFarm", {Title = "AutoFarm", Default = false})
AutoFarmToggle:OnChanged(function()
    safeSetG("AutoFarm", AutoFarmToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["AutoFarm"]) == "function" and AutoFarmToggle.Value then _G["AutoFarm"]() end
    end)
end)

local AutoFarmCakeToggle = Tabs.Auto:AddToggle("AutoFarmCake", {Title = "AutoFarmCake", Default = false})
AutoFarmCakeToggle:OnChanged(function()
    safeSetG("AutoFarmCake", AutoFarmCakeToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["AutoFarmCake"]) == "function" and AutoFarmCakeToggle.Value then _G["AutoFarmCake"]() end
    end)
end)

local FarmChestToggle = Tabs.Auto:AddToggle("FarmChest", {Title = "FarmChest", Default = false})
FarmChestToggle:OnChanged(function()
    safeSetG("FarmChest", FarmChestToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["FarmChest"]) == "function" and FarmChestToggle.Value then _G["FarmChest"]() end
    end)
end)

local CollectBerryToggle = Tabs.Auto:AddToggle("CollectBerry", {Title = "CollectBerry", Default = false})
CollectBerryToggle:OnChanged(function()
    safeSetG("CollectBerry", CollectBerryToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["CollectBerry"]) == "function" and CollectBerryToggle.Value then _G["CollectBerry"]() end
    end)
end)

local BringMobsToggle = Tabs.Auto:AddToggle("BringMobs", {Title = "BringMobs", Default = false})
BringMobsToggle:OnChanged(function()
    safeSetG("BringMobs", BringMobsToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["BringMobs"]) == "function" and BringMobsToggle.Value then _G["BringMobs"]() end
    end)
end)

local SelectMaterialDropdown = Tabs.Misc:AddDropdown("SelectMaterial", {Title = "SelectMaterial", Values = {"Radiactive Material", "Leather + Scrap Metal", "Magma Ore", "Fish Tail", "Angel Wings", "Mystic Droplet", "Vampire Fang", "Gunpowder", "Mini Tusk", "Conjured Cocoa"}, Multi = false, Default = 1})
SelectMaterialDropdown:OnChanged(function(Value) safeSetG("SelectMaterial", Value) end)

local TeamDropdown = Tabs.Misc:AddDropdown("Team", {Title = "Team", Values = {"None", "Pirates", "Marines"}, Multi = false, Default = 1})
TeamDropdown:OnChanged(function(Value) safeSetG("Team", Value) end)

local StopTweenToggle = Tabs.Auto:AddToggle("StopTween", {Title = "StopTween", Default = false})
StopTweenToggle:OnChanged(function()
    safeSetG("StopTween", StopTweenToggle.Value)
    pcall(function()
        -- try to call a global function of same name if it exists
        if type(_G["StopTween"]) == "function" and StopTweenToggle.Value then _G["StopTween"]() end
    end)
end)

local tg_Aimbot_Gun = Tabs.Settings:AddToggle("Aimbot_Gun", {Title = "Aimbot_Gun", Default = false})
tg_Aimbot_Gun:OnChanged(function() safeSetG("Aimbot_Gun", tg_Aimbot_Gun.Value) end)

local tg_Aimbot_Skill = Tabs.Settings:AddToggle("Aimbot_Skill", {Title = "Aimbot_Skill", Default = false})
tg_Aimbot_Skill:OnChanged(function() safeSetG("Aimbot_Skill", tg_Aimbot_Skill.Value) end)

local tg_AutoAzuerEmber = Tabs.Settings:AddToggle("AutoAzuerEmber", {Title = "AutoAzuerEmber", Default = false})
tg_AutoAzuerEmber:OnChanged(function() safeSetG("AutoAzuerEmber", tg_AutoAzuerEmber.Value) end)

local tg_AutoBartilo = Tabs.Settings:AddToggle("AutoBartilo", {Title = "AutoBartilo", Default = false})
tg_AutoBartilo:OnChanged(function() safeSetG("AutoBartilo", tg_AutoBartilo.Value) end)

local tg_AutoBlazeEmber = Tabs.Settings:AddToggle("AutoBlazeEmber", {Title = "AutoBlazeEmber", Default = false})
tg_AutoBlazeEmber:OnChanged(function() safeSetG("AutoBlazeEmber", tg_AutoBlazeEmber.Value) end)

local tg_AutoBoss = Tabs.Settings:AddToggle("AutoBoss", {Title = "AutoBoss", Default = false})
tg_AutoBoss:OnChanged(function() safeSetG("AutoBoss", tg_AutoBoss.Value) end)

local tg_AutoBuyChip = Tabs.Settings:AddToggle("AutoBuyChip", {Title = "AutoBuyChip", Default = false})
tg_AutoBuyChip:OnChanged(function() safeSetG("AutoBuyChip", tg_AutoBuyChip.Value) end)

local tg_AutoBuyEnchancementColour = Tabs.Settings:AddToggle("AutoBuyEnchancementColour", {Title = "AutoBuyEnchancementColour", Default = false})
tg_AutoBuyEnchancementColour:OnChanged(function() safeSetG("AutoBuyEnchancementColour", tg_AutoBuyEnchancementColour.Value) end)

local tg_AutoBuyLegendarySword = Tabs.Settings:AddToggle("AutoBuyLegendarySword", {Title = "AutoBuyLegendarySword", Default = false})
tg_AutoBuyLegendarySword:OnChanged(function() safeSetG("AutoBuyLegendarySword", tg_AutoBuyLegendarySword.Value) end)

local tg_AutoCollectBone = Tabs.Settings:AddToggle("AutoCollectBone", {Title = "AutoCollectBone", Default = false})
tg_AutoCollectBone:OnChanged(function() safeSetG("AutoCollectBone", tg_AutoCollectBone.Value) end)

local tg_AutoDarkBoss = Tabs.Settings:AddToggle("AutoDarkBoss", {Title = "AutoDarkBoss", Default = false})
tg_AutoDarkBoss:OnChanged(function() safeSetG("AutoDarkBoss", tg_AutoDarkBoss.Value) end)

local tg_AutoDooHee = Tabs.Settings:AddToggle("AutoDooHee", {Title = "AutoDooHee", Default = false})
tg_AutoDooHee:OnChanged(function() safeSetG("AutoDooHee", tg_AutoDooHee.Value) end)

local tg_AutoEliteHunterHop = Tabs.Settings:AddToggle("AutoEliteHunterHop", {Title = "AutoEliteHunterHop", Default = false})
tg_AutoEliteHunterHop:OnChanged(function() safeSetG("AutoEliteHunterHop", tg_AutoEliteHunterHop.Value) end)

local tg_AutoElitehunter = Tabs.Settings:AddToggle("AutoElitehunter", {Title = "AutoElitehunter", Default = false})
tg_AutoElitehunter:OnChanged(function() safeSetG("AutoElitehunter", tg_AutoElitehunter.Value) end)

local tg_AutoFactory = Tabs.Settings:AddToggle("AutoFactory", {Title = "AutoFactory", Default = false})
tg_AutoFactory:OnChanged(function() safeSetG("AutoFactory", tg_AutoFactory.Value) end)

local tg_AutoFarmFruits = Tabs.Settings:AddToggle("AutoFarmFruits", {Title = "AutoFarmFruits", Default = false})
tg_AutoFarmFruits:OnChanged(function() safeSetG("AutoFarmFruits", tg_AutoFarmFruits.Value) end)

local tg_AutoFarmMaterial = Tabs.Settings:AddToggle("AutoFarmMaterial", {Title = "AutoFarmMaterial", Default = false})
tg_AutoFarmMaterial:OnChanged(function() safeSetG("AutoFarmMaterial", tg_AutoFarmMaterial.Value) end)

local tg_AutoFindPrehistoric = Tabs.Settings:AddToggle("AutoFindPrehistoric", {Title = "AutoFindPrehistoric", Default = false})
tg_AutoFindPrehistoric:OnChanged(function() safeSetG("AutoFindPrehistoric", tg_AutoFindPrehistoric.Value) end)

local tg_AutoGetTushita = Tabs.Settings:AddToggle("AutoGetTushita", {Title = "AutoGetTushita", Default = false})
tg_AutoGetTushita:OnChanged(function() safeSetG("AutoGetTushita", tg_AutoGetTushita.Value) end)

local tg_AutoHaki = Tabs.Settings:AddToggle("AutoHaki", {Title = "AutoHaki", Default = false})
tg_AutoHaki:OnChanged(function() safeSetG("AutoHaki", tg_AutoHaki.Value) end)

local tg_AutoHolyTorch = Tabs.Settings:AddToggle("AutoHolyTorch", {Title = "AutoHolyTorch", Default = false})
tg_AutoHolyTorch:OnChanged(function() safeSetG("AutoHolyTorch", tg_AutoHolyTorch.Value) end)

local tg_AutoHydraTree = Tabs.Settings:AddToggle("AutoHydraTree", {Title = "AutoHydraTree", Default = false})
tg_AutoHydraTree:OnChanged(function() safeSetG("AutoHydraTree", tg_AutoHydraTree.Value) end)

local tg_AutoKillFishCrew = Tabs.Settings:AddToggle("AutoKillFishCrew", {Title = "AutoKillFishCrew", Default = false})
tg_AutoKillFishCrew:OnChanged(function() safeSetG("AutoKillFishCrew", tg_AutoKillFishCrew.Value) end)

local tg_AutoKillPiranha = Tabs.Settings:AddToggle("AutoKillPiranha", {Title = "AutoKillPiranha", Default = false})
tg_AutoKillPiranha:OnChanged(function() safeSetG("AutoKillPiranha", tg_AutoKillPiranha.Value) end)

local tg_AutoKillPlayer = Tabs.Settings:AddToggle("AutoKillPlayer", {Title = "AutoKillPlayer", Default = false})
tg_AutoKillPlayer:OnChanged(function() safeSetG("AutoKillPlayer", tg_AutoKillPlayer.Value) end)

local tg_AutoKillShark = Tabs.Settings:AddToggle("AutoKillShark", {Title = "AutoKillShark", Default = false})
tg_AutoKillShark:OnChanged(function() safeSetG("AutoKillShark", tg_AutoKillShark.Value) end)

local tg_AutoLawRaid = Tabs.Settings:AddToggle("AutoLawRaid", {Title = "AutoLawRaid", Default = false})
tg_AutoLawRaid:OnChanged(function() safeSetG("AutoLawRaid", tg_AutoLawRaid.Value) end)

local tg_AutoMateria = Tabs.Settings:AddToggle("AutoMateria", {Title = "AutoMateria", Default = false})
tg_AutoMateria:OnChanged(function() safeSetG("AutoMateria", tg_AutoMateria.Value) end)

local tg_AutoMobDragon = Tabs.Settings:AddToggle("AutoMobDragon", {Title = "AutoMobDragon", Default = false})
tg_AutoMobDragon:OnChanged(function() safeSetG("AutoMobDragon", tg_AutoMobDragon.Value) end)

local tg_AutoMysticIsland = Tabs.Settings:AddToggle("AutoMysticIsland", {Title = "AutoMysticIsland", Default = false})
tg_AutoMysticIsland:OnChanged(function() safeSetG("AutoMysticIsland", tg_AutoMysticIsland.Value) end)

local tg_AutoNear = Tabs.Settings:AddToggle("AutoNear", {Title = "AutoNear", Default = false})
tg_AutoNear:OnChanged(function() safeSetG("AutoNear", tg_AutoNear.Value) end)

local tg_AutoPlayerHunter = Tabs.Settings:AddToggle("AutoPlayerHunter", {Title = "AutoPlayerHunter", Default = false})
tg_AutoPlayerHunter:OnChanged(function() safeSetG("AutoPlayerHunter", tg_AutoPlayerHunter.Value) end)

local tg_AutoQuestRace = Tabs.Settings:AddToggle("AutoQuestRace", {Title = "AutoQuestRace", Default = false})
tg_AutoQuestRace:OnChanged(function() safeSetG("AutoQuestRace", tg_AutoQuestRace.Value) end)

local tg_AutoQuestYama = Tabs.Settings:AddToggle("AutoQuestYama", {Title = "AutoQuestYama", Default = false})
tg_AutoQuestYama:OnChanged(function() safeSetG("AutoQuestYama", tg_AutoQuestYama.Value) end)

local tg_AutoRaceV3 = Tabs.Settings:AddToggle("AutoRaceV3", {Title = "AutoRaceV3", Default = false})
tg_AutoRaceV3:OnChanged(function() safeSetG("AutoRaceV3", tg_AutoRaceV3.Value) end)

local tg_AutoRaceV4 = Tabs.Settings:AddToggle("AutoRaceV4", {Title = "AutoRaceV4", Default = false})
tg_AutoRaceV4:OnChanged(function() safeSetG("AutoRaceV4", tg_AutoRaceV4.Value) end)

local tg_AutoRaidPirate = Tabs.Settings:AddToggle("AutoRaidPirate", {Title = "AutoRaidPirate", Default = false})
tg_AutoRaidPirate:OnChanged(function() safeSetG("AutoRaidPirate", tg_AutoRaidPirate.Value) end)

local tg_AutoRengoku = Tabs.Settings:AddToggle("AutoRengoku", {Title = "AutoRengoku", Default = false})
tg_AutoRengoku:OnChanged(function() safeSetG("AutoRengoku", tg_AutoRengoku.Value) end)

local tg_AutoSecondSea = Tabs.Settings:AddToggle("AutoSecondSea", {Title = "AutoSecondSea", Default = false})
tg_AutoSecondSea:OnChanged(function() safeSetG("AutoSecondSea", tg_AutoSecondSea.Value) end)

local tg_AutoSkill = Tabs.Settings:AddToggle("AutoSkill", {Title = "AutoSkill", Default = false})
tg_AutoSkill:OnChanged(function() safeSetG("AutoSkill", tg_AutoSkill.Value) end)

local tg_AutoStoreFruit = Tabs.Settings:AddToggle("AutoStoreFruit", {Title = "AutoStoreFruit", Default = false})
tg_AutoStoreFruit:OnChanged(function() safeSetG("AutoStoreFruit", tg_AutoStoreFruit.Value) end)

local tg_AutoTerrorshark = Tabs.Settings:AddToggle("AutoTerrorshark", {Title = "AutoTerrorshark", Default = false})
tg_AutoTerrorshark:OnChanged(function() safeSetG("AutoTerrorshark", tg_AutoTerrorshark.Value) end)

local tg_AutoYama = Tabs.Settings:AddToggle("AutoYama", {Title = "AutoYama", Default = false})
tg_AutoYama:OnChanged(function() safeSetG("AutoYama", tg_AutoYama.Value) end)

local tg_AutoYamaQuest = Tabs.Settings:AddToggle("AutoYamaQuest", {Title = "AutoYamaQuest", Default = false})
tg_AutoYamaQuest:OnChanged(function() safeSetG("AutoYamaQuest", tg_AutoYamaQuest.Value) end)

local tg_Autofruit = Tabs.Settings:AddToggle("Autofruit", {Title = "Autofruit", Default = false})
tg_Autofruit:OnChanged(function() safeSetG("Autofruit", tg_Autofruit.Value) end)

local tg_Autopole = Tabs.Settings:AddToggle("Autopole", {Title = "Autopole", Default = false})
tg_Autopole:OnChanged(function() safeSetG("Autopole", tg_Autopole.Value) end)

local tg_Autosaw = Tabs.Settings:AddToggle("Autosaw", {Title = "Autosaw", Default = false})
tg_Autosaw:OnChanged(function() safeSetG("Autosaw", tg_Autosaw.Value) end)

local tg_Autoterrorshark = Tabs.Settings:AddToggle("Autoterrorshark", {Title = "Autoterrorshark", Default = false})
tg_Autoterrorshark:OnChanged(function() safeSetG("Autoterrorshark", tg_Autoterrorshark.Value) end)

local tg_BringMob = Tabs.Settings:AddToggle("BringMob", {Title = "BringMob", Default = false})
tg_BringMob:OnChanged(function() safeSetG("BringMob", tg_BringMob.Value) end)

local tg_BringMonster = Tabs.Settings:AddToggle("BringMonster", {Title = "BringMonster", Default = false})
tg_BringMonster:OnChanged(function() safeSetG("BringMonster", tg_BringMonster.Value) end)

local tg_CheckPoint = Tabs.Settings:AddToggle("CheckPoint", {Title = "CheckPoint", Default = false})
tg_CheckPoint:OnChanged(function() safeSetG("CheckPoint", tg_CheckPoint.Value) end)

local tg_ChestESP = Tabs.Settings:AddToggle("ChestESP", {Title = "ChestESP", Default = false})
tg_ChestESP:OnChanged(function() safeSetG("ChestESP", tg_ChestESP.Value) end)

local tg_ChiefWarden = Tabs.Settings:AddToggle("ChiefWarden", {Title = "ChiefWarden", Default = false})
tg_ChiefWarden:OnChanged(function() safeSetG("ChiefWarden", tg_ChiefWarden.Value) end)

local tg_Clip = Tabs.Settings:AddToggle("Clip", {Title = "Clip", Default = false})
tg_Clip:OnChanged(function() safeSetG("Clip", tg_Clip.Value) end)

local tg_CodeSelect = Tabs.Settings:AddToggle("CodeSelect", {Title = "CodeSelect", Default = false})
tg_CodeSelect:OnChanged(function() safeSetG("CodeSelect", tg_CodeSelect.Value) end)

local tg_CollectBerryHop = Tabs.Settings:AddToggle("CollectBerryHop", {Title = "CollectBerryHop", Default = false})
tg_CollectBerryHop:OnChanged(function() safeSetG("CollectBerryHop", tg_CollectBerryHop.Value) end)

local tg_CollectEgg = Tabs.Settings:AddToggle("CollectEgg", {Title = "CollectEgg", Default = false})
tg_CollectEgg:OnChanged(function() safeSetG("CollectEgg", tg_CollectEgg.Value) end)

local tg_Color = Tabs.Settings:AddToggle("Color", {Title = "Color", Default = false})
tg_Color:OnChanged(function() safeSetG("Color", tg_Color.Value) end)

local tg_CursedCaptain = Tabs.Settings:AddToggle("CursedCaptain", {Title = "CursedCaptain", Default = false})
tg_CursedCaptain:OnChanged(function() safeSetG("CursedCaptain", tg_CursedCaptain.Value) end)

local tg_DefendVolcano = Tabs.Settings:AddToggle("DefendVolcano", {Title = "DefendVolcano", Default = false})
tg_DefendVolcano:OnChanged(function() safeSetG("DefendVolcano", tg_DefendVolcano.Value) end)

local tg_Dungeon = Tabs.Settings:AddToggle("Dungeon", {Title = "Dungeon", Default = false})
tg_Dungeon:OnChanged(function() safeSetG("Dungeon", tg_Dungeon.Value) end)

local tg_EnabledPvP = Tabs.Settings:AddToggle("EnabledPvP", {Title = "EnabledPvP", Default = false})
tg_EnabledPvP:OnChanged(function() safeSetG("EnabledPvP", tg_EnabledPvP.Value) end)

local tg_FarmBone = Tabs.Settings:AddToggle("FarmBone", {Title = "FarmBone", Default = false})
tg_FarmBone:OnChanged(function() safeSetG("FarmBone", tg_FarmBone.Value) end)

local tg_FarmCake = Tabs.Settings:AddToggle("FarmCake", {Title = "FarmCake", Default = false})
tg_FarmCake:OnChanged(function() safeSetG("FarmCake", tg_FarmCake.Value) end)

local tg_FarmChocola = Tabs.Settings:AddToggle("FarmChocola", {Title = "FarmChocola", Default = false})
tg_FarmChocola:OnChanged(function() safeSetG("FarmChocola", tg_FarmChocola.Value) end)

local tg_FarmDaiBan = Tabs.Settings:AddToggle("FarmDaiBan", {Title = "FarmDaiBan", Default = false})
tg_FarmDaiBan:OnChanged(function() safeSetG("FarmDaiBan", tg_FarmDaiBan.Value) end)

local tg_FastAttackDelay = Tabs.Settings:AddToggle("FastAttackDelay", {Title = "FastAttackDelay", Default = false})
tg_FastAttackDelay:OnChanged(function() safeSetG("FastAttackDelay", tg_FastAttackDelay.Value) end)

local tg_Fast_Delay = Tabs.Settings:AddToggle("Fast_Delay", {Title = "Fast_Delay", Default = false})
tg_Fast_Delay:OnChanged(function() safeSetG("Fast_Delay", tg_Fast_Delay.Value) end)

local tg_Fullykatakuri = Tabs.Settings:AddToggle("Fullykatakuri", {Title = "Fullykatakuri", Default = false})
tg_Fullykatakuri:OnChanged(function() safeSetG("Fullykatakuri", tg_Fullykatakuri.Value) end)

local tg_Grabfruit = Tabs.Settings:AddToggle("Grabfruit", {Title = "Grabfruit", Default = false})
tg_Grabfruit:OnChanged(function() safeSetG("Grabfruit", tg_Grabfruit.Value) end)

local tg_GravityBlade = Tabs.Settings:AddToggle("GravityBlade", {Title = "GravityBlade", Default = false})
tg_GravityBlade:OnChanged(function() safeSetG("GravityBlade", tg_GravityBlade.Value) end)

local tg_Greybeard = Tabs.Settings:AddToggle("Greybeard", {Title = "Greybeard", Default = false})
tg_Greybeard:OnChanged(function() safeSetG("Greybeard", tg_Greybeard.Value) end)

local tg_Greybeardhop = Tabs.Settings:AddToggle("Greybeardhop", {Title = "Greybeardhop", Default = false})
tg_Greybeardhop:OnChanged(function() safeSetG("Greybeardhop", tg_Greybeardhop.Value) end)

local tg_Hallow = Tabs.Settings:AddToggle("Hallow", {Title = "Hallow", Default = false})
tg_Hallow:OnChanged(function() safeSetG("Hallow", tg_Hallow.Value) end)

local tg_KillFishCrew = Tabs.Settings:AddToggle("KillFishCrew", {Title = "KillFishCrew", Default = false})
tg_KillFishCrew:OnChanged(function() safeSetG("KillFishCrew", tg_KillFishCrew.Value) end)

local tg_KillGolem = Tabs.Settings:AddToggle("KillGolem", {Title = "KillGolem", Default = false})
tg_KillGolem:OnChanged(function() safeSetG("KillGolem", tg_KillGolem.Value) end)

local tg_KillPiranha = Tabs.Settings:AddToggle("KillPiranha", {Title = "KillPiranha", Default = false})
tg_KillPiranha:OnChanged(function() safeSetG("KillPiranha", tg_KillPiranha.Value) end)

local tg_KillShark = Tabs.Settings:AddToggle("KillShark", {Title = "KillShark", Default = false})
tg_KillShark:OnChanged(function() safeSetG("KillShark", tg_KillShark.Value) end)

local tg_Kill_Aura = Tabs.Settings:AddToggle("Kill_Aura", {Title = "Kill_Aura", Default = false})
tg_Kill_Aura:OnChanged(function() safeSetG("Kill_Aura", tg_Kill_Aura.Value) end)

local tg_LockTween = Tabs.Settings:AddToggle("LockTween", {Title = "LockTween", Default = false})
tg_LockTween:OnChanged(function() safeSetG("LockTween", tg_LockTween.Value) end)

local tg_Logo = Tabs.Settings:AddToggle("Logo", {Title = "Logo", Default = false})
tg_Logo:OnChanged(function() safeSetG("Logo", tg_Logo.Value) end)

local tg_Longsword = Tabs.Settings:AddToggle("Longsword", {Title = "Longsword", Default = false})
tg_Longsword:OnChanged(function() safeSetG("Longsword", tg_Longsword.Value) end)

local tg_NameHub = Tabs.Settings:AddToggle("NameHub", {Title = "NameHub", Default = false})
tg_NameHub:OnChanged(function() safeSetG("NameHub", tg_NameHub.Value) end)

local tg_Nocliprock = Tabs.Settings:AddToggle("Nocliprock", {Title = "Nocliprock", Default = false})
tg_Nocliprock:OnChanged(function() safeSetG("Nocliprock", tg_Nocliprock.Value) end)

local tg_NotAutoEquip = Tabs.Settings:AddToggle("NotAutoEquip", {Title = "NotAutoEquip", Default = false})
tg_NotAutoEquip:OnChanged(function() safeSetG("NotAutoEquip", tg_NotAutoEquip.Value) end)

local tg_Pray = Tabs.Settings:AddToggle("Pray", {Title = "Pray", Default = false})
tg_Pray:OnChanged(function() safeSetG("Pray", tg_Pray.Value) end)

local tg_RandomAuto = Tabs.Settings:AddToggle("RandomAuto", {Title = "RandomAuto", Default = false})
tg_RandomAuto:OnChanged(function() safeSetG("RandomAuto", tg_RandomAuto.Value) end)

local tg_Rdbone = Tabs.Settings:AddToggle("Rdbone", {Title = "Rdbone", Default = false})
tg_Rdbone:OnChanged(function() safeSetG("Rdbone", tg_Rdbone.Value) end)

local tg_RipIndraKill = Tabs.Settings:AddToggle("RipIndraKill", {Title = "RipIndraKill", Default = false})
tg_RipIndraKill:OnChanged(function() safeSetG("RipIndraKill", tg_RipIndraKill.Value) end)

local tg_SafeMode = Tabs.Settings:AddToggle("SafeMode", {Title = "SafeMode", Default = false})
tg_SafeMode:OnChanged(function() safeSetG("SafeMode", tg_SafeMode.Value) end)

local tg_SailBoat = Tabs.Settings:AddToggle("SailBoat", {Title = "SailBoat", Default = false})
tg_SailBoat:OnChanged(function() safeSetG("SailBoat", tg_SailBoat.Value) end)

local tg_SelectBoat = Tabs.Settings:AddToggle("SelectBoat", {Title = "SelectBoat", Default = false})
tg_SelectBoat:OnChanged(function() safeSetG("SelectBoat", tg_SelectBoat.Value) end)

local tg_SelectBoss = Tabs.Settings:AddToggle("SelectBoss", {Title = "SelectBoss", Default = false})
tg_SelectBoss:OnChanged(function() safeSetG("SelectBoss", tg_SelectBoss.Value) end)

local tg_SelectIsland = Tabs.Settings:AddToggle("SelectIsland", {Title = "SelectIsland", Default = false})
tg_SelectIsland:OnChanged(function() safeSetG("SelectIsland", tg_SelectIsland.Value) end)

local tg_SelectNPC = Tabs.Settings:AddToggle("SelectNPC", {Title = "SelectNPC", Default = false})
tg_SelectNPC:OnChanged(function() safeSetG("SelectNPC", tg_SelectNPC.Value) end)

local tg_SelectPly = Tabs.Settings:AddToggle("SelectPly", {Title = "SelectPly", Default = false})
tg_SelectPly:OnChanged(function() safeSetG("SelectPly", tg_SelectPly.Value) end)

local tg_SelectQuestDragon = Tabs.Settings:AddToggle("SelectQuestDragon", {Title = "SelectQuestDragon", Default = false})
tg_SelectQuestDragon:OnChanged(function() safeSetG("SelectQuestDragon", tg_SelectQuestDragon.Value) end)

local tg_SelectWeapon = Tabs.Settings:AddToggle("SelectWeapon", {Title = "SelectWeapon", Default = false})
tg_SelectWeapon:OnChanged(function() safeSetG("SelectWeapon", tg_SelectWeapon.Value) end)

local tg_SelectWeaponGun = Tabs.Settings:AddToggle("SelectWeaponGun", {Title = "SelectWeaponGun", Default = false})
tg_SelectWeaponGun:OnChanged(function() safeSetG("SelectWeaponGun", tg_SelectWeaponGun.Value) end)

local tg_Selectweapon = Tabs.Settings:AddToggle("Selectweapon", {Title = "Selectweapon", Default = false})
tg_Selectweapon:OnChanged(function() safeSetG("Selectweapon", tg_Selectweapon.Value) end)

local tg_SpinPos = Tabs.Settings:AddToggle("SpinPos", {Title = "SpinPos", Default = false})
tg_SpinPos:OnChanged(function() safeSetG("SpinPos", tg_SpinPos.Value) end)

local tg_StartRaid = Tabs.Settings:AddToggle("StartRaid", {Title = "StartRaid", Default = false})
tg_StartRaid:OnChanged(function() safeSetG("StartRaid", tg_StartRaid.Value) end)

local tg_SwodCanvande = Tabs.Settings:AddToggle("SwodCanvande", {Title = "SwodCanvande", Default = false})
tg_SwodCanvande:OnChanged(function() safeSetG("SwodCanvande", tg_SwodCanvande.Value) end)

local tg_SwodCanvander = Tabs.Settings:AddToggle("SwodCanvander", {Title = "SwodCanvander", Default = false})
tg_SwodCanvander:OnChanged(function() safeSetG("SwodCanvander", tg_SwodCanvander.Value) end)

local tg_SwodTwinHooks = Tabs.Settings:AddToggle("SwodTwinHooks", {Title = "SwodTwinHooks", Default = false})
tg_SwodTwinHooks:OnChanged(function() safeSetG("SwodTwinHooks", tg_SwodTwinHooks.Value) end)

local tg_SwodsBuddy = Tabs.Settings:AddToggle("SwodsBuddy", {Title = "SwodsBuddy", Default = false})
tg_SwodsBuddy:OnChanged(function() safeSetG("SwodsBuddy", tg_SwodsBuddy.Value) end)

local tg_SwodsDRTrident = Tabs.Settings:AddToggle("SwodsDRTrident", {Title = "SwodsDRTrident", Default = false})
tg_SwodsDRTrident:OnChanged(function() safeSetG("SwodsDRTrident", tg_SwodsDRTrident.Value) end)

local tg_SwodsFlail = Tabs.Settings:AddToggle("SwodsFlail", {Title = "SwodsFlail", Default = false})
tg_SwodsFlail:OnChanged(function() safeSetG("SwodsFlail", tg_SwodsFlail.Value) end)

local tg_TPB = Tabs.Settings:AddToggle("TPB", {Title = "TPB", Default = false})
tg_TPB:OnChanged(function() safeSetG("TPB", tg_TPB.Value) end)

local tg_TargTrial = Tabs.Settings:AddToggle("TargTrial", {Title = "TargTrial", Default = false})
tg_TargTrial:OnChanged(function() safeSetG("TargTrial", tg_TargTrial.Value) end)

local tg_TeleportIsland = Tabs.Settings:AddToggle("TeleportIsland", {Title = "TeleportIsland", Default = false})
tg_TeleportIsland:OnChanged(function() safeSetG("TeleportIsland", tg_TeleportIsland.Value) end)

local tg_TeleportNPC = Tabs.Settings:AddToggle("TeleportNPC", {Title = "TeleportNPC", Default = false})
tg_TeleportNPC:OnChanged(function() safeSetG("TeleportNPC", tg_TeleportNPC.Value) end)

local tg_TeleportPly = Tabs.Settings:AddToggle("TeleportPly", {Title = "TeleportPly", Default = false})
tg_TeleportPly:OnChanged(function() safeSetG("TeleportPly", tg_TeleportPly.Value) end)

local tg_ThirdSea = Tabs.Settings:AddToggle("ThirdSea", {Title = "ThirdSea", Default = false})
tg_ThirdSea:OnChanged(function() safeSetG("ThirdSea", tg_ThirdSea.Value) end)

local tg_Title = Tabs.Settings:AddToggle("Title", {Title = "Title", Default = false})
tg_Title:OnChanged(function() safeSetG("Title", tg_Title.Value) end)

local tg_TpPrehistoric = Tabs.Settings:AddToggle("TpPrehistoric", {Title = "TpPrehistoric", Default = false})
tg_TpPrehistoric:OnChanged(function() safeSetG("TpPrehistoric", tg_TpPrehistoric.Value) end)

local tg_Trident = Tabs.Settings:AddToggle("Trident", {Title = "Trident", Default = false})
tg_Trident:OnChanged(function() safeSetG("Trident", tg_Trident.Value) end)

local tg_Trylux = Tabs.Settings:AddToggle("Trylux", {Title = "Trylux", Default = false})
tg_Trylux:OnChanged(function() safeSetG("Trylux", tg_Trylux.Value) end)

local tg_TweenFruit = Tabs.Settings:AddToggle("TweenFruit", {Title = "TweenFruit", Default = false})
tg_TweenFruit:OnChanged(function() safeSetG("TweenFruit", tg_TweenFruit.Value) end)

local tg_TweenMGear = Tabs.Settings:AddToggle("TweenMGear", {Title = "TweenMGear", Default = false})
tg_TweenMGear:OnChanged(function() safeSetG("TweenMGear", tg_TweenMGear.Value) end)

local tg_TweenToKitsune = Tabs.Settings:AddToggle("TweenToKitsune", {Title = "TweenToKitsune", Default = false})
tg_TweenToKitsune:OnChanged(function() safeSetG("TweenToKitsune", tg_TweenToKitsune.Value) end)

local tg_TweenVolcano = Tabs.Settings:AddToggle("TweenVolcano", {Title = "TweenVolcano", Default = false})
tg_TweenVolcano:OnChanged(function() safeSetG("TweenVolcano", tg_TweenVolcano.Value) end)

local tg_UseGun = Tabs.Settings:AddToggle("UseGun", {Title = "UseGun", Default = false})
tg_UseGun:OnChanged(function() safeSetG("UseGun", tg_UseGun.Value) end)

local tg_UseMelee = Tabs.Settings:AddToggle("UseMelee", {Title = "UseMelee", Default = false})
tg_UseMelee:OnChanged(function() safeSetG("UseMelee", tg_UseMelee.Value) end)

local tg_UseSkill = Tabs.Settings:AddToggle("UseSkill", {Title = "UseSkill", Default = false})
tg_UseSkill:OnChanged(function() safeSetG("UseSkill", tg_UseSkill.Value) end)

local tg_UseSword = Tabs.Settings:AddToggle("UseSword", {Title = "UseSword", Default = false})
tg_UseSword:OnChanged(function() safeSetG("UseSword", tg_UseSword.Value) end)

local tg_WalkWater = Tabs.Settings:AddToggle("WalkWater", {Title = "WalkWater", Default = false})
tg_WalkWater:OnChanged(function() safeSetG("WalkWater", tg_WalkWater.Value) end)

local tg_XaiSkillC = Tabs.Settings:AddToggle("XaiSkillC", {Title = "XaiSkillC", Default = false})
tg_XaiSkillC:OnChanged(function() safeSetG("XaiSkillC", tg_XaiSkillC.Value) end)

local tg_XaiSkillX = Tabs.Settings:AddToggle("XaiSkillX", {Title = "XaiSkillX", Default = false})
tg_XaiSkillX:OnChanged(function() safeSetG("XaiSkillX", tg_XaiSkillX.Value) end)

local tg_XaiSkillZ = Tabs.Settings:AddToggle("XaiSkillZ", {Title = "XaiSkillZ", Default = false})
tg_XaiSkillZ:OnChanged(function() safeSetG("XaiSkillZ", tg_XaiSkillZ.Value) end)

local tg_chat = Tabs.Settings:AddToggle("chat", {Title = "chat", Default = false})
tg_chat:OnChanged(function() safeSetG("chat", tg_chat.Value) end)

local tg_dao = Tabs.Settings:AddToggle("dao", {Title = "dao", Default = false})
tg_dao:OnChanged(function() safeSetG("dao", tg_dao.Value) end)

local tg_findVolcanicMagnet = Tabs.Settings:AddToggle("findVolcanicMagnet", {Title = "findVolcanicMagnet", Default = false})
tg_findVolcanicMagnet:OnChanged(function() safeSetG("findVolcanicMagnet", tg_findVolcanicMagnet.Value) end)

local tg_imageLogo = Tabs.Settings:AddToggle("imageLogo", {Title = "imageLogo", Default = false})
tg_imageLogo:OnChanged(function() safeSetG("imageLogo", tg_imageLogo.Value) end)

local tg_leaderboard = Tabs.Settings:AddToggle("leaderboard", {Title = "leaderboard", Default = false})
tg_leaderboard:OnChanged(function() safeSetG("leaderboard", tg_leaderboard.Value) end)

local tg_selectFruitFarm = Tabs.Settings:AddToggle("selectFruitFarm", {Title = "selectFruitFarm", Default = false})
tg_selectFruitFarm:OnChanged(function() safeSetG("selectFruitFarm", tg_selectFruitFarm.Value) end)


-- Manual actions
Tabs.Main:AddButton({Title="Stop Tween", Description="Try to call StopTween or set _G.StopTween=true", Callback=function()
    pcall(function()
        if type(StopTween) == "function" then StopTween() else safeSetG("StopTween", true) end
    end)
end})

Tabs.Main:AddButton({Title="Toggle FastAttack (call)", Description="Toggle _G.FastAttack and attempt to start FastAttack loop if present", Callback=function()
    local new = not (_G.FastAttack == true)
    safeSetG("FastAttack", new)
    Fluent:Notify({Title="FastAttack", Content = "Set _G.FastAttack = "..tostring(new), Duration=4})
    pcall(function() if type(_ENV and _ENV.rz_FastAttack) == "table" and type(_ENV.rz_FastAttack.AttackNearest) == "function" then _ENV.rz_FastAttack:AttackNearest() end end)
end})
...
InterfaceManager:SetFolder("Matsune_Fluent_Rewritten")
SaveManager:SetFolder("Matsune_Fluent_Rewritten/specific-game")
InterfaceManager:BuildInterfaceSection(Tabs.Settings)
SaveManager:BuildConfigSection(Tabs.Settings)

Window:SelectTab(1)
Fluent:Notify({Title="Matsune (Fluent)", Content="Interface ready. Original script loaded with suppressed GUI.", Duration=6})
